...
parser.add_argument(
    '--imgScale', type = int, default=18, help='input image size (# feat in conv4), 14 means input image size is 224 * 224 ')
...
with torch.no_grad() : 
    RefFeat = torch.cuda.FloatTensor(args.nbSaveRefFeat, 8, 256, args.imgScale, args.imgScale).zero_()
    QueryFeat = torch.cuda.FloatTensor(1, 1, 256, args.imgScale, args.imgScale).zero_()
    ...
    for i in tqdm(range(nbGroup)) : 
        ...
        for j, sourceImgName in tqdm(enumerate(label['val'])) :
            ...
            score = torch.sum(torch.sum(torch.sum(RefFeat * QueryFeat, dim=2), dim=2), dim=2) / args.imgScale / args.imgScale # nbSaveRefFeat * 8
